...
from dtpo.dtpo_continuous_action import DTPOLearner
...
parser.add_argument(
    "--env-name", type=str, default="Pendulum-v1", help="the name of the environment"
)
parser.add_argument(
...
parser.add_argument(
    "--output-dir",
    type=str,
    default="out_continuous",
    help="the directory to output result files into",
)
parser.add_argument(
    "--num-discrete-actions",
    type=int,
    default=2,
    help="number of discrete actions (only for the Pendulum-v1-BangBang environment)",
)
parser.add_argument(
    "--grad-clip-norm",
    type=float,
...
env, env_params = make_env_from_name(args.env_name, seed=args.seed, num_discrete_actions=args.num_discrete_actions)
...
env_to_feature_action_names = {
    "Pendulum-v1": (["cos theta", "sin theta", "theta dot"], ["torque_mu", "torque_sigma"]),
}
...
filename = f"{experiment_dir}/continuous_tree"
export_tree(
...
if isinstance(env, gymnax.environments.environment.Environment):
    for repetition in tqdm(range(args.evaluation_rollouts)):
        ...
        for i in range(env_params.max_steps_in_episode):
            ...
            logits = model.tree_policy_.apply(model.best_params_, obs)
            mean, log_sigma = np.split(np.array(logits), 2, axis=-1)
            action = float(mean)
